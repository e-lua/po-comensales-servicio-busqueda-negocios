...
func Pg_Find_BasicData(idbusiness int) (models.Pg_BasicData_ToBusiness, error) {
	...
	q := "SELECT COALESCE(b.typesuscription,b.uniquename,'sin nombre'),COALESCE(b.name,'sin nombre'), COALESCE(b.timezone,'0'),COALESCE(CASE WHEN now()::time at time zone CONCAT('UTC',(b.timezone::integer*-1)::varchar(3))< concat(bsch.endtime,b.timezone)::time with time zone AND now()::time at time zone CONCAT('UTC',(b.timezone::integer*-1)::varchar(3))> concat(bsch.starttime,b.timezone)::time with time zone THEN true ELSE false END,'false') FROM business as b JOIN businessschedule as bsch ON b.idbusiness=bsch.idbusiness WHERE bsch.idschedule=EXTRACT(ISODOW FROM (NOW()::timestamp at time zone CONCAT('UTC',(b.timezone::integer)::varchar(3)))) AND b.idbusiness=$1"
...
func Pg_Find_BasicData_WithoutData(idbusiness int) (models.Pg_BasicData_ToBusiness, error) {
	...
	db := models.Conectar_Pg_DB()
	q := "SELECT COALESCE(b.typesuscription,b.uniquename,'sin nombre'),COALESCE(b.name,'sin nombre'),COALESCE(b.timezone,'0'),false FROM business as b WHERE b.idbusiness=$1"
	error_shown := db.QueryRow(ctx, q, idbusiness).Scan(&basic_data.Typesuscription, &basic_data.Uniquename, &basic_data.Name, &basic_data.TimeZone, &basic_data.IsOpen)
